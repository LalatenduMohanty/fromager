...
def simple_dir(tmp_path: pathlib.Path) -> pathlib.Path:
    ...
    basedir = tmp_path / "simple"
    basedir.mkdir()
    project_dir = basedir / "testpkg"
    project_dir.mkdir()
    _create_fake_wheel(project_dir, "testpkg-1.0-py3-none-any.whl")
    (project_dir / "testpkg-1.0-py3-none-any.whl.metadata").write_bytes(
        b"Metadata-Version: 2.1\nName: testpkg\nVersion: 1.0\n"
    )
    (project_dir / "testpkg-1.0.tar.gz").write_bytes(b"fake tarball")
    return basedir
...
def test_update_wheel_mirror_moves_to_downloads(
    ...
    assert not (tmp_context.wheels_build / "foo-1.0-py3-none-any.whl").exists()
    assert (tmp_context.wheels_downloads / "foo-1.0-py3-none-any.whl").exists()
...
def test_update_wheel_mirror_creates_symlink(
    ...
    symlink = tmp_context.wheel_server_dir / "foo" / "foo-1.0-py3-none-any.whl"
...
def test_update_wheel_mirror_prebuilt_wheels(
    ...
    symlink = tmp_context.wheel_server_dir / "bar" / "bar-2.0-py3-none-any.whl"
    assert symlink.is_symlink()
    assert symlink.is_file()
...
def test_serve_file_bad_extension(
    ...
    (simple_dir / "testpkg" / "bad.txt").write_text("bad")
